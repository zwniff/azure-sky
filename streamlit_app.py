...
for i in range(len(example)):
    if st.sidebar.button(f'Fill with example data {i+1}'):
        st.session_state['Pollutant_PM2.5_µg/m³'] = example.iloc[i]['Pollutant_PM2.5_µg/m³']
        st.session_state['Pollutant_PM10_µg/m³'] = example.iloc[i]['Pollutant_PM10_µg/m³']
        st.session_state['Pollutant_O3_ppb'] = example.iloc[i]['Pollutant_O3_ppb']
        st.session_state['Pollutant_NO2_ppb'] = example.iloc[i]['Pollutant_NO2_ppb']
        st.session_state['Pollutant_CO_ppm'] = example.iloc[i]['Pollutant_CO_ppm']
        st.session_state['Pollutant_SO2_ppb'] = example.iloc[i]['Pollutant_SO2_ppb']
        st.session_state['UrbanVegetationArea_m2'] = example.iloc[i]['UrbanVegetationArea_m2']
        st.session_state['Humidity_%'] = example.iloc[i]['Humidity_%']
        st.session_state['AirTemperature_C'] = example.iloc[i]['AirTemperature_C']
        st.session_state['AnnualEnergySavings_%'] = example.iloc[i]['AnnualEnergySavings_%']
        st.session_state['PopulationDensity_people/km²'] = example.iloc[i]['PopulationDensity_people/km²']
        st.session_state['RenewableEnergyPercentage_%'] = example.iloc[i]['RenewableEnergyPercentage_%']
        st.session_state['AnnualEnergyConsumption_kWh'] = example.iloc[i]['AnnualEnergyConsumption_kWh']
        st.session_state['GreenSpaceIndex_%'] = example.iloc[i]['GreenSpaceIndex_%']
        st.session_state['HistoricPollutantLevels'] = example.iloc[i]['HistoricPollutantLevels']